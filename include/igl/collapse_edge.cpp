  ...
  int & a_f2)
{
  ...
  assert(s<d && "s should be less than d");
  // move source and destination to placement
Eigen::RowVector3d vs = V.row(s);
Eigen::RowVector3d vd = V.row(d);

//find nearest point on edge from p

Eigen::RowVector3d v1 = vs - p;
Eigen::RowVector3d v2 = vd - p;
Eigen::RowVector3d v3 = vd - vs;

double t = -v1.dot(v3) / v3.dot(v3);
t = std::max(0.0, std::min(1.0, t));
Eigen::RowVector3d p1 = (1-t) * V_undeformed.row(s) + t * V_undeformed.row(d);
V_undeformed.row(s) = p1;
V_undeformed.row(d) = p1;
  ...
  int & f2)
{
  ...
  if(pre_collapse(V,F,E,EMAP,EF,EI,Q,EQ,C,e))
  {
    collapsed = collapse_edge(
      e,C.row(e),
      Nsv,Nsf,Ndv,Ndf,
      V,V_undeformed, F,E,EMAP,EF,EI,e1,e2,f1,f2);
  }else
  {
    // Aborted by pre collapse callback
    collapsed = false;
  ...
  if(collapsed)
  {
    ...
    for(auto & ei : Ne)
    {
       ...
       double cost;
       RowVectorXd place;
       cost_and_placement(ei,V_undeformed,F,E,EMAP,EF,EI,cost,place);
       // Increment timestamp
       EQ(ei)++;
       // Replace in queue
       Q.emplace(cost,ei,EQ(ei));
